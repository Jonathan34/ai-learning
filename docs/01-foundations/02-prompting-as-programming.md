...
One thing this chapter assumes: that the prompt is the hard part. In real systems, it usually isn't. The prompt might be 5% of what the model actually sees — the rest is retrieved documents, conversation history, tool outputs, and user state. The next chapter covers that bigger problem.
